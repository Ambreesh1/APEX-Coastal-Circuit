...
(function (A) {
  ...
  class Car {
    constructor(track, id) {
      this.id=id; this.name=DRIVERS[id][0]; this.color=DRIVERS[id][1];
  ...
  function drive(car,input,dt,track) {
    ...
    const grip=car.offroad?3.7:(car.drifting?2.4:11);
    lateral*=Math.exp(-grip*dt);
    const accel=car.throttle*16.5*Math.max(.14,1-Math.max(0,forward)/90);
    const resistance=.6+forward*forward*.0015+(car.offroad?8:0)+(car.drifting?1.3:0);
    forward=Math.max(0,forward+(accel-resistance-car.brake*31)*dt);
    forward=Math.min(forward,78);
    ...
    if(Math.abs(car.near.offset)>limit){
      ...
      if(car.cooldown===0 && speed>3){car.vx*=.77;car.vz*=.77;car.impact=A.clamp(speed/50,.2,1);car.cooldown=.55;}
      car.near=track.nearest(car.x,car.z,car.near.index);
    }
  }
  function collide(a,b) {
    ...
    for(const sa of [-1.15,1.15])for(const sb of [-1.15,1.15]){
      ...
      if(closing<0){
        ...
        for(const c of [a,b])if(c.cooldown===0){c.impact=A.clamp(-closing/25,.15,.8);c.cooldown=.35;}
      }
    }
  }
  function aiInput(car,cars,track,pace=1) {
    const speed=Math.hypot(car.vx,car.vz),s=car.near.s;
    let lane=car.lane,targetSpeed=64*pace;
    for(let d=8;d<=100;d+=12){
      const curve=Math.abs(track.at(s+d).curve);
      const cornerSpeed=Math.sqrt(14/Math.max(.001,curve))*pace;
      targetSpeed=Math.min(targetSpeed,Math.sqrt(cornerSpeed*cornerSpeed+2*13*Math.max(0,d-14)));
    }
    ...
    const look=12+speed*.43,p=track.at(s+look,A.clamp(lane,-5.5,5.5));
    const desired=Math.atan2(p.x-car.x,p.z-car.z),error=A.angle(desired-car.heading);
    const steeringScale=Math.max(.4,speed/2.8*(.38/(1+speed*.045)));
  ...
  class Race {
    constructor(track,pace='sport') {
      this.track=track;this.cars=DRIVERS.map((_,i)=>new Car(track,i));this.player=this.cars[0];
      this.time=0;this.pace={rookie:.73,sport:.87,pro:1.02}[pace]||.87;
      this.events=[];this.totalLaps=3;
    }
    step(dt,input) {
